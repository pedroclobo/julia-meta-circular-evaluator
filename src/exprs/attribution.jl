...
is_local_definition(expr, env) =
    (is_variable_attribution(expr) && !has_name_in_local_frames(expr.args[1], env)) ||
    (is_function_attribution(expr) && !has_name_in_local_frames(expr.args[1].args[1], env)) ||
    (is_fexpr_attribution(expr) && !has_name_in_local_frames(expr.args[1].args[1], env)) ||
    (is_macro_attribution(expr) && !has_name_in_local_frames(expr.args[1].args[1], env))
    ...
    begin
        let init = eval(init(expr, env), env)
            if is_local_assignment(expr, env)
                replace_binding!(env, name(expr, env), init)
